...
public final class ConsultantDashboardController {

  private final ConsultantDashboardQueryService queryService;

  public ConsultantDashboardController(ConsultantDashboardQueryService queryService) {
    this.queryService = Objects.requireNonNull(queryService, "queryService must not be null");
  }

  @GetMapping
  public ResponseEntity<ApiResponseEnvelope<ApiSafeResponseBody>> loadDashboard(
      @AuthenticationPrincipal RtoAuthenticatedPrincipal principal) {
    requireConsultantRole(principal.portalRole());
    return ResponseEntity.ok(ApiResponseEnvelope.success(queryService.load(
        buildAccessRequest(), principal.organizationId())));
  }
