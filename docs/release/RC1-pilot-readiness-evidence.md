...
| Docker/Testcontainers | Release owner | `rtk docker version`; `rtk docker info`; `rtk docker desktop update --check-only` | Fri Jun 12 16:22:23 CST 2026 | exit 0; Docker client/server reachable; client 29.5.3 on context `desktop-linux`; Docker Desktop 4.77.0 (228796), already latest; Engine/Server Version 29.5.3; server `docker-desktop`, Docker Desktop linux/aarch64; `docker info` observed 2 containers, 0 running, 2 stopped, and 5 images, with no containers created or started by this task | passed | E0 | docs/release/rc1-artifacts/RC1-02-docker.txt | not-applicable |  | none; read-only Docker inspection; no containers started | Proceed to RC1-03 Migration validation gate |
...
| Frontend regression | Frontend owner | `rtk npm --workspace @rto/web run test` | Fri Jun 12 16:35:10 CST 2026 | exit 0; Vitest web suite reported 10 test files passed and 45 tests passed; suite includes `portalRouteContract.test.ts` and new `portalRenderSmoke.test.tsx` | passed | E1 | docs/release/rc1-artifacts/RC1-04B-web-test.txt | not-applicable |  | none | Proceed to RC1-05 Backend gates only after RC1-04B evidence is reviewed |
| Frontend route preservation | Frontend owner | `portalRouteContract.test.ts` and `portalRenderSmoke.test.tsx` within `rtk npm --workspace @rto/web run test` | Fri Jun 12 16:35:10 CST 2026 | exit 0; source-level contract still checks v2.1/v2.0 owner, unified consultant, client, candidate, and admin portal route strings; render-level jsdom smoke renders `/owner`, `/consultant`, `/client`, `/candidate`, and `/admin` through real `App` route wiring, with `/candidate` asserting candidate sign-in content rather than global nav text; no Playwright/browser E2E was run | passed | E1 | docs/release/rc1-artifacts/RC1-04B-web-test.txt | not-applicable |  | none | Proceed to RC1-05 Backend gates; keep browser/user-flow claims for later E4 gates |
| Frontend typecheck | Frontend owner | `rtk npm run typecheck:web` | Fri Jun 12 16:35:10 CST 2026 | exit 0; `tsc --noEmit --pretty false` completed with no reported diagnostics | passed | E1 | docs/release/rc1-artifacts/RC1-04B-web-typecheck.txt | not-applicable |  | none | Proceed to RC1-05 Backend gates only after RC1-04B evidence is reviewed |
| Frontend build | Frontend owner | `rtk npm run build:web` | Fri Jun 12 16:35:10 CST 2026 | exit 0; `tsc --noEmit && vite build` completed; Vite transformed 84 modules and emitted `dist/index.html`, `dist/assets/index-DOR-Kjww.css`, and `dist/assets/index-BGt5O77o.js` | passed | E1 | docs/release/rc1-artifacts/RC1-04B-web-build.txt | not-applicable |  | none | Proceed to RC1-05 Backend gates only after RC1-04B evidence is reviewed |
...
| --- | --- | --- | --- | --- | --- | --- |
| RC1-04-B1 | medium | route-usability gap | RC1-04B added `apps/web/src/portalRenderSmoke.test.tsx`; `rtk npm --workspace @rto/web run test` exited 0 with 10 test files and 45 tests, including five jsdom render smoke cases for `/owner`, `/consultant`, `/client`, `/candidate`, and `/admin` through real `App` route wiring. The `/candidate` smoke now asserts candidate sign-in content rather than global nav text. This closes the render-level gap but is not E4 browser evidence. | Frontend owner | No further RC1-04B action; keep browser/user-flow route readiness claims for later Playwright/browser gates | closed |
